...
if (args.help) {
  console.log(processName + ': a simple set of Google Search proxies\n' +
       '\tOptions:\n' +
  ...
  try {
    serp.json(p, (data) => {
      ...
      related.forEach((rel, index) => {
        ...
        var newLink = rel.query.replace(/\s/g, '+')
        // console.log(new_link);
        rel.link = '/get_query/' + newLink
      })
      // let results = data.local_results
      if (args.verbose > 3) {
        console.log(data)
      }
      res.render('search', { data: data, json: jsonString, related_searches2: related })
      // res.sendFile( __dirname + "/" + "search3.html" ); // search = post
    })
...
app.post('/process_git', urlencodedParser, function (req, res) {
  ...
  googleIt(options, { 'query': response.query }).then(results => {
    ...
    res.render('search-git', { data: data, json: jsonString })
    // res.sendFile( __dirname + "/" + "search3.html" ); // search = post
  }).catch(e => {
...
app.post('/googler_process', urlencodedParser, function (req, res) {
  ...
  request(args.api + '/json?q=' + response.query,
    { json: true }, (err, res2, results) => {
      ...
      } else if (res2.statusCode > 399) { // Heroku uses 304
